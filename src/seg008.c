...
//	word image_flipped;
	...
	*/
	...
	if (blit_flip) {
		xpos -= image->w/*width*/;
#ifdef NSPIRE
		/* Cache flipped sprites in chtab[n_images + id] */
		{
			chtab_type* chtab = chtab_addrs[chtab_id];
			if (chtab && image_id < chtab->n_images) {
				int flip_idx = chtab->n_images + image_id;
				if (chtab->images[flip_idx] == NULL) {
					chtab->images[flip_idx] = hflip(image);
				}
				image = chtab->images[flip_idx];
				need_free_image = 0;
			} else {
				need_free_image = 1;
				image = hflip(image);
