...
#ifdef NSPIRE
	/* Read from potentially unaligned packed struct via memcpy */
	short move_time, curr_move;
	memcpy(&move_time, &moves_ptr[demoindex].time, sizeof(short));
	if (move_time <= demo_time) {
		++demo_index;
	} else {
		demoindex = demo_index - 1;
	}
	memcpy(&curr_move, &moves_ptr[demoindex].move, sizeof(short));
#else
