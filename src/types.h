...
/* On Nspire, use __attribute__((packed)) instead of #pragma pack
 * because GCC generates safe unaligned access code for attributed structs
 * but not reliably for #pragma pack on ARM926EJ-S. */
#ifdef NSPIRE
...
#ifdef NSPIRE
/* On Nspire (no audio), provide a dummy stb_vorbis type */
...
#ifdef NSPIRE
  /* 16-bit RGB565 - matches Nspire CX screen format */
  #define Rmsk 0xF800
  #define Gmsk 0x07E0
  #define Bmsk 0x001F
  #define Amsk 0x0000
  #define NSPIRE_BPP 16
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) rgb_type {
#else
#pragma pack(push,1)
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) dat_pal_type {
#else
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) dat_shpl_type {
#else
...
typedef PACKED_STRUCT(auto_move_type) {
	short time,move;
} auto_move_type;
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) dat_header_type {
#else
#pragma pack(push,1)
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) dat_res_type {
#else
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) dat_table_type {
#else
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) image_data_type {
#else
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) rawfont_type {
#else
#pragma pack(push,1)
...
//misaligned data == CRASH!! on PSP and Nspire
#if !defined(__PSP__) && !defined(NSPIRE)
#pragma pack(push,1)
#endif
#ifdef NSPIRE
typedef struct __attribute__((packed)) fixes_options_type {
#else
...
#ifdef NSPIRE
typedef struct __attribute__((packed)) custom_options_type {
#else
