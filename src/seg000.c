...
#ifdef NSPIRE
/* Cleanup all game resources at exit - called from restore_stuff() */
void cleanup_game_resources(void) {
	free_peels();
	free_all_chtabs_from(0);
	release_title_images();
	if (guard_palettes) { free(guard_palettes); guard_palettes = NULL; }
	if (level_var_palettes) { free(level_var_palettes); level_var_palettes = NULL; }
	if (offscreen_surface) { free_surface(offscreen_surface); offscreen_surface = NULL; }
}
#endif
...
void show_title() {
	...
	load_opt_sounds(sound_50_story_2_princess, sound_55_story_1_absence);
	idle();
	do_paused();
...
void transition_ltr() {
#ifdef NSPIRE
	/* On Nspire: instant transition, no animation.
	 * Avoids 160× idle()/update_screen() calls with partial palette. */
...
#ifdef NSPIRE
	return; /* No audio on Nspire - skip loading sound DAT files entirely */
#endif
