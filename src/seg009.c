...
#include "dirent.h"
#include <libndls.h>
extern void nspire_set_gr_mode(byte grmode);
extern void nspire_update_screen(void);
extern void nspire_poll_input(void);
extern int nspire_digi_unavailable;
#elif defined(_WIN32)
#include <windows.h>
#include <wchar.h>
#else
...
void restore_stuff() {
#ifdef NSPIRE
	/* Free all game resources before SDL_Quit */
	extern void cleanup_game_resources(void);
	cleanup_game_resources();

	/* Free font */
	if (hc_font.chtab) { free_chtab(hc_font.chtab); hc_font.chtab = NULL; }

	/* Free onscreen surface */
	if (onscreen_surface_) { SDL_FreeSurface(onscreen_surface_); onscreen_surface_ = NULL; }

	/* Close any remaining open DAT files */
	while (dat_chain_ptr != NULL) {
		close_dat(dat_chain_ptr);
	}

	/* Free copyprot dialog if allocated */
	if (copyprot_dialog) {
		if (copyprot_dialog->peel) free_peel(copyprot_dialog->peel);
		free(copyprot_dialog);
...
#ifdef NSPIRE
	size_t alloc_size = sizeof(chtab_type) + sizeof(void *) * n_images * 2;
#else
...
#ifdef NSPIRE
	for (word id = 0; id < n_images * 2; ++id) {
#else
...
image_type* decode_image(image_data_type* image_data, dat_pal_type* palette) {
	 ...
	 * Some sprites (guard, sword) have empty embedded palettes -
	 * their colors come from the global VGA palette set later.
	 * SDL will handle 8-bit → 16-bit blit conversion dynamically. */
#endif
	return image;
}
...
surface_type* make_offscreen_buffer(const rect_type* rect) {
	...
	if (onscreen_surface_ != NULL) {
		return SDL_CreateRGBSurface(0, rect->right, rect->bottom, 16,
			onscreen_surface_->format->Rmask, onscreen_surface_->format->Gmask,
			onscreen_surface_->format->Bmask, 0);
	}
	return SDL_CreateRGBSurface(0, rect->right, rect->bottom, NSPIRE_BPP, Rmsk, Gmsk, Bmsk, 0);
...
#ifdef NSPIRE
	/* On Nspire: just delay. The actual blit from offscreen→onscreen
	 * is done by the subsequent method_1_blit_rect in show_title().
	 * This avoids showing intermediate palette states. */
	(void)source_surface; (void)which_rows;
	SDL_Delay(500);
#else
...
#ifdef NSPIRE
	/* Nspire: fill with real black (pixel value 0), don't rely on palette */
	{
		SDL_Rect r = {0, 0, 320, 200};
		SDL_FillRect(onscreen_surface_, &r, 0);
	}
	update_screen();
#else
...
peel_type* read_peel_from_screen(const rect_type* rect) {
	...
	SDL_Surface* peel_surface = SDL_CreateRGBSurface(0, rect->right - rect->left, rect->bottom - rect->top,
		16, onscreen_surface_->format->Rmask, onscreen_surface_->format->Gmask, onscreen_surface_->format->Bmask, 0);
#elif !defined(USE_ALPHA)
	SDL_Surface* peel_surface = SDL_CreateRGBSurface(0, rect->right - rect->left, rect->bottom - rect->top,
	                                                 24, Rmsk, Gmsk, Bmsk, 0);
#else
...
#ifndef NSPIRE /* Begin audio implementation - not needed on Nspire */
...
#endif /* !NSPIRE - End of audio implementation */

#ifdef NSPIRE
/* Audio stub implementations for Nspire */
void play_sound_from_buffer(sound_buffer_type* buffer) { (void)buffer; }
void turn_music_on_off(byte new_state) { enable_music = new_state; }
void turn_sound_on_off(byte new_state) { is_sound_on = new_state; }
int check_sound_playing() { return 0; }
void free_sound(sound_buffer_type* buffer) { if (buffer) free(buffer); }
sound_buffer_type* load_sound(int index) { (void)index; return NULL; }
#endif
...
#ifdef NSPIRE
	/* On Nspire: create a 16-bit mono-colored copy of the sprite. */
	{
		SDL_Surface* colored_image = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, 16,
			current_target_surface->format->Rmask,
			current_target_surface->format->Gmask,
			current_target_surface->format->Bmask, 0);
		if (colored_image == NULL) { return image; }

		rgb_type palette_color = palette[color];
		Uint16 rgb_color16 = (Uint16)SDL_MapRGB(colored_image->format,
			palette_color.r<<2, palette_color.g<<2, palette_color.b<<2);
		Uint16 colorkey16 = 0;

		SDL_FillRect(colored_image, NULL, colorkey16);
		SDL_LockSurface(image);
		SDL_LockSurface(colored_image);
		int src_stride = image->pitch;
		int dst_stride = colored_image->pitch;
		for (int y = 0; y < h; ++y) {
			Uint8* src_row = (Uint8*)image->pixels + src_stride * y;
			Uint16* dst_row = (Uint16*)((byte*)colored_image->pixels + dst_stride * y);
			for (int x = 0; x < w; ++x) {
				if (src_row[x] != 0) {
					dst_row[x] = rgb_color16;
				}
			}
		}
		SDL_UnlockSurface(colored_image);
		SDL_UnlockSurface(image);
		SDL_SetColorKey(colored_image, SDL_TRUE, colorkey16);
...
#ifdef NSPIRE
	/* No overlay on Nspire, just fill with solid color */
	SDL_Rect dest_rect;
	rect_to_sdlrect(rect, &dest_rect);
	rgb_type palette_color = palette[color];
...
#ifdef NSPIRE
	/* Simplified: use SDL_FillRect for border lines */
...
#ifdef NSPIRE
	SDL_Surface* helper_surface = SDL_CreateRGBSurface(0, dest_rect->w, dest_rect->h, 16,
		onscreen_surface_->format->Rmask, onscreen_surface_->format->Gmask, onscreen_surface_->format->Bmask, 0);
#else
...
#endif /* !NSPIRE - end of process_events */
